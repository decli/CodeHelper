...
## 2.6 第二次真机走查：字重才是主因（v1.4.2）

v1.4.1 把码区宽度还回去之后，维护者反馈「取件码还是比之前小很多，字体也不如之前容易看清」。复查发现真正的主因不是间距，而是**字重**：

v1.4 把取件码与首页大数字的字重从 v1.3.1 的 `ExtraBold`(800) 提到了 `Black`(900)。系统中文字体（Roboto / Noto Sans CJK / MiSans）多数没有真实的 900 字面，请求 900 会触发**合成加粗**——笔画被机械加粗、字碗（0 / 3 / 8 中间的洞）被填小，深色模式下浅色字本来就会向外发散，两者叠加就是「更糊、更难认」。同时 900 的字面更宽，实测放大时只能选到更小的字号，于是「更粗」和「更小」一起发生。

已改回 v1.3.1 的 `ExtraBold`，并把剩下的宽度也让给字号：

| 版本 | 字重 | 页边距 / 码区内边距 / 字间距 | 10 位码估算字号 |
| --- | --- | --- | --- |
| v1.3.1 | ExtraBold | 16 / 12 / 1sp | ≈69sp |
| v1.4.0 | Black | 20 / 16 / 2sp | ≈60sp |
| v1.4.1 | Black | 20 / 4 / 1sp | ≈66sp |
| **v1.4.2** | **ExtraBold** | **16 / 4 / 1sp** | **≈72sp** |

同一次走查修掉的另外两处：

- **卡头时间被截断成「09-03 …」**。字阶上调后这一行在大系统字号下放不下。改为：驿站名和时间拆成两段，只有驿站名会省略号，时间永远完整；分组打开时（默认）分组标题已经写了驿站名，卡头干脆只显示时间。
- **「1 个」与「最近 7 天」胶囊下沿没对齐**。104sp 数字的文本框底部比数字视觉下沿低了一个字体下伸空间（约 25dp），胶囊按底对齐就会掉下去。改为把数字行的布局高度裁到它的第一条基线（数字没有下伸部，基线即视觉下沿），胶囊再按 `Alignment.Bottom` 排版即可严丝合缝，不需要写死偏移量。

## 2.7 版本记录

| 版本 | 内容 |
| --- | --- |
| `1.4.0` | 按 v1.4 设计包实现：英雄区去卡片化、取件卡重构、按驿站分组、出示页、语音播报、首次引导、设置即改即存、提示条改造，外加规范手势表里的下拉刷新 |
| `1.4.1` | 真机走查第一轮：字阶整体上调一档、正文字重 400→500、码区内边距 16→4dp、字间距按码长分档、上限 96→104sp、出示页码铺满整屏、卡内文字按钮改 64dp 竖排 |
| `1.4.2` | 真机走查第二轮：取件码与首页大数字字重 900→800（回到 v1.3.1），首页列表边距 20→16dp，卡头时间不再被截断，时间胶囊与数字下沿基线对齐 |

`docs/ui-design-spec-v1.4.md` 描述的是 `1.4.2` 的实现口径。
